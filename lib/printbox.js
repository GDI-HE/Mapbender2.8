...
*/
		...
		if (map.epsg === 'EPSG:4326') {
			var centerLat = (parseFloat(coordsArray[1]) + parseFloat(coordsArray[3])) / 2;
			var latRad = centerLat * Math.PI / 180.0;
			xtentxInM = xtentx * (2.0 * Math.PI * 6378137.0 / 360.0) * Math.cos(latRad);
		}
...
*/
			...
			if (map.epsg === 'EPSG:4326') {
				...
				var latRad = centerMap.y * Math.PI / 180.0;
				halfW = realWidthInM * degPerMeter / Math.cos(latRad);
