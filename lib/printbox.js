...
//	var map = Mapbender.modules[target];
	...
	var circleWidth = options.circleWidth || 4;
        ...
        // when true: suppress the rectangle frame, rotation circle, and corner handles;
        // only the draggable center dot is painted (spotlight overlay replaces the visuals)
        var noFrame = options.noFrame || false;
	...
	var centerMap = options.realCenter || null;
		...
		$circleCanvas.css("z-index", "110").mousedown(function (e) {
			...
			$("#" + target).mousemove(function (e) {
				...
				if (Math.abs(diffAngle) >= 1) {
					angle = currentAngle;
					// Subtract diffAngle: getAngle() returns negative for CW drag in screen
					// space, so subtracting makes totalAngle increase for CW drag, consistent
					// with the negated rotate() which is now CW-positive.
...
*/
		...
		var xtentxInM = xtentx;
		if (map.epsg === 'EPSG:4326') {
			// Convert degrees to metres using the N-S (latitude) scale only.
			// No cos(lat) here so that setScale/getScale stay consistent and
			// the bbox aspect ratio matches the paper (portrait stays portrait).
			xtentxInM = xtentx * (2.0 * Math.PI * 6378137.0 / 360.0);
		}
		scale = parseInt(Math.round(xtentxInM / (printWidth / 100)), 10);
		return scale;
	};
...
*/
			...
			// fixed:true but centerMap not yet set (e.g. first call on normal print
			// before setCenterMap() has been called) — fall back to map centre pixel.
			if (!centerMap) {
				centerMap = convertPixelToMap(getCenter());
			}

			var halfW = realWidthInM;
			var halfH = realHeightInM;
			if (map.epsg === 'EPSG:4326') {
				var degPerMeter = 360.0 / (2.0 * Math.PI * 6378137.0);
				// Use the same N-S scale for both axes so the bbox aspect ratio
				// matches the paper (portrait stays portrait on screen).
				halfW = realWidthInM * degPerMeter;
				halfH = realHeightInM * degPerMeter;
			}

			var newMin = new Point(centerMap.x - 0.5 * halfW, centerMap.y - 0.5 * halfH);
			var newMax = new Point(centerMap.x + 0.5 * halfW, centerMap.y + 0.5 * halfH);
	...
	this.rotate = function (degree) {
		// Negate so that positive degree = clockwise rotation on screen,
		// matching the PDF backend (Imagick rotateImage = CW for positive angle).
	...
	this.paintPoints = function () {
		// corner handles suppressed in noFrame / spotlight mode
		if (noFrame) { return; }
	...
	this.paintBox = function () {
		...
		// rotation circle — suppressed in noFrame / spotlight mode
		circleCanvas.clear();
		if (!noFrame) {
		...
		// center dot — always drawn so the box remains draggable
		boxCanvas.clear();
		boxCanvas.setColor(pointColour);
		boxCanvas.fillEllipse(c.x - 4, c.y - 4, 10, 10);

		// rectangle frame — suppressed in noFrame / spotlight mode
		if (!noFrame) {
	 ...
	 */
	/**
	 * Updates the real-world center anchor of the print box.
	 * Call setScale() afterwards to repaint at the new position.
	 * @param {Object} worldPoint  Object with .x and .y real-world coordinates
	 */
	this.setCenterMap = function (worldPoint) {
		centerMap = worldPoint;
	};
