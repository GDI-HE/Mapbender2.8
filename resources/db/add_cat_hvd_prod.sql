-- Entfernen von Einträgen mit fkey_custom_category_id = 13 (empty category)
DELETE FROM mb_metadata_custom_category
WHERE fkey_custom_category_id = 13;

-- Einfügen für fkey_custom_category_id = 18 (Geospatial)
INSERT INTO mb_metadata_custom_category (fkey_metadata_id, fkey_custom_category_id)
SELECT fkey_metadata_id, 18
FROM md_termsofuse 
...
AND NOT EXISTS (
    ...
    AND fkey_custom_category_id = 18
);

-- Einfügen für fkey_custom_category_id = 19 (Mobility)
INSERT INTO mb_metadata_custom_category (fkey_metadata_id, fkey_custom_category_id)
SELECT fkey_metadata_id, 19
FROM md_termsofuse 
WHERE fkey_termsofuse_id IN (
    SELECT termsofuse_id 
    FROM termsofuse 
...
AND NOT EXISTS (
    ...
    FROM mb_metadata_custom_category 
    WHERE fkey_metadata_id = md_termsofuse.fkey_metadata_id 
    AND fkey_custom_category_id = 19
);

-- Einfügen für fkey_custom_category_id = 16 (Earth observation and environment)
INSERT INTO mb_metadata_custom_category (fkey_metadata_id, fkey_custom_category_id)
SELECT fkey_metadata_id, 16
FROM md_termsofuse 
WHERE fkey_termsofuse_id IN (
    SELECT termsofuse_id 
    FROM termsofuse 
...
AND NOT EXISTS (
    ...
    FROM mb_metadata_custom_category 
    WHERE fkey_metadata_id = md_termsofuse.fkey_metadata_id 
    AND fkey_custom_category_id = 16
);
