...
if (count($ownguis)>0){
	...
	if(isset($guiList) && $guiList!=""){
		...
		$res = db_prep_query($sql,$v,$t);
		$count=0;
		echo"<label style='margin-top: 15px;' for='wfsList'>Bereits enthaltene WFS</label><select class='form-control' name='wfsList' size='5' readonly>";
		...
		echo"<label style='margin-top: 15px;' for='wfsList'>Bereits enthaltene WFS</label><select class='form-control' name='wfsList' on Click='submit()'>";
		echo "</select></div>";
	}
	...
	if (isset($xml_file)){
		...
		echo "<span id='helpBlock' class='help-block bg-danger' style='padding:10px;margin-top:25px;word-wrap:break-word;border-radius:4px;'>Die URL muss ein valides WFS Capabilities Dokument liefern. In der Regel sollte folgendes in Ihrer URL enthalten sein:<p style='margin:10px 0 0 0;font-weight:bold;'>REQUEST=GetCapabilities&SERVICE=WFS&VERSION=2.0.0 (oder 1.1.0)</p></span></div>";
	}else{
