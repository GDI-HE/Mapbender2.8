...
*/
			...
			//Some lines were deleted due to this change -> In case of issues compare this against rlp branch	
				...
				$datasetFileIdentifier = $datasetMatrix[$i]['fileidentifier'];
				if ($downloadOptionsArray[$datasetFileIdentifier] != null) {
					$this->datasetJSON->dataset->srv[$i]->coupledResources->inspireAtomFeeds = json_decode($downloadOptionsArray[$datasetFileIdentifier]);
				}
				$downloadOptionsFromMetadata = json_decode(getDownloadOptions(array($datasetFileIdentifier), $this->protocol . "://" . $this->hostName . "/mapbender/", $this->protocol . "://" . $this->hostName));
				if (isset($downloadOptionsFromMetadata->{$datasetFileIdentifier}) && isset($downloadOptionsFromMetadata->{$datasetFileIdentifier}->option)) {
					$optionSignatureArray = array();
					if (isset($this->datasetJSON->dataset->srv[$i]->coupledResources->inspireAtomFeeds) && is_array($this->datasetJSON->dataset->srv[$i]->coupledResources->inspireAtomFeeds)) {
						foreach ($this->datasetJSON->dataset->srv[$i]->coupledResources->inspireAtomFeeds as $existingOption) {
							$optionSignatureArray[] = md5(
								(string)$existingOption->type . "|" .
								(string)$existingOption->serviceTitle . "|" .
								(string)$existingOption->accessClient . "|" .
								(string)$existingOption->accessUrl . "|" .
								(string)$existingOption->link . "|" .
								(string)$existingOption->serviceId . "|" .
								(string)$existingOption->resourceId
							);
						}
					}
					foreach ($downloadOptionsFromMetadata->{$datasetFileIdentifier}->option as $dlOption) {
						if ($dlOption->type == "downloadlink" || $dlOption->type == "distribution" || $dlOption->type == "remotelist" || (!isset($this->datasetJSON->dataset->srv[$i]->coupledResources->inspireAtomFeeds) && ($dlOption->type == "ogcapifeatures" || $dlOption->type == "directwfs"))) {
							$optionSignature = md5(
								(string)$dlOption->type . "|" .
								(string)$dlOption->serviceTitle . "|" .
								(string)$dlOption->accessClient . "|" .
								(string)$dlOption->accessUrl . "|" .
								(string)$dlOption->link . "|" .
								(string)$dlOption->serviceId . "|" .
								(string)$dlOption->resourceId
							);
							if (!in_array($optionSignature, $optionSignatureArray, true)) {
								$this->datasetJSON->dataset->srv[$i]->coupledResources->inspireAtomFeeds[] = $dlOption;
								$optionSignatureArray[] = $optionSignature;
							}
						}
					}
				}
